...
%for jj = 2
    ...
    depths_by_actions = zeros(1,length(actions_array));
    explored_actions = [];
    ...
    for ii = 1:n_trials
        ...
        theta_posterior = updateTheta(action, response(1), theta_prior1, forward_NN);
        entropy_array(jj,ii) = entropy_calculation(theta_posterior);
        theta_prior1 = theta_posterior;
        depths_by_actions = update_depths_by_actions(actions_array, action, response(1), depths_by_actions);
...
function Posterior_distribution = ABC_Method(observed_data, threshold, action, prior, forward_NN)
    ...
    disp(observed_data)
    % calcalute the distance from Y 
    distance = distance_function(X(:,1),observed_data);
    %disp('X:');disp(X);
    %disp('observed:');disp(observed_data);
    %disp(distance);
...
function response = comsol_response(comsol_model, action)
    ...
    % add noise
    response = response + [noise_sd*randn; 0];
end
...
function action = eGreedy(iter, n, epsilon, theta_prior, actions, expected_depths, observed_depths, forward_NN, d_target)
    ...
    %if iter > (1-epsilon)*n
        ...
        for i = 1:length(actions)
            ...
            if observed_depths(i) == 0  % not observed before
                depth = expected_depths(1,i);
            else
