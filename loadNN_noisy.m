...
epsilon_array = 0.5;
action_history = zeros(length(epsilon_array),n_trials);
response_history = zeros(length(epsilon_array),n_trials);
response_array = zeros(length(epsilon_array),n_trials);
entropy_array = zeros(length(epsilon_array),n_trials);
...
%for jj = 2
    ...
    for ii = 1:n_trials
        ...
        action_history(jj,ii) = action;
        [response, response_noisy] = comsol_response(comsol_model, action);
        response_history(jj,ii) = response(2);
        fprintf('response: %4.4f\n',response)
        fprintf('response noisy: %4.4f\n',response_noisy)
        response_array(jj,ii) = response_noisy(2);
        theta_posterior = updateTheta(action, response_noisy, theta_prior1, forward_NN);
        entropy_array(jj,ii) = entropy_calculation(theta_posterior);
        theta_prior1 = theta_posterior;
        depths_by_actions = update_depths_by_actions(actions_array, action, response_noisy, depths_by_actions);
...
%
depthmse = zeros(1,size(response_history,1));
variance = zeros(1,size(response_history,1));
for i = 1:size(response_history,1)
    depthmse(i) = sum(abs(response_history(i,:)-d_target));
...
Y = [-0.03*ones(n_trials+1,1), [response_history(1,:),response_history(end)]', [response_array(1,:),response_array(end)]'];
h = stairs(X,Y);
h(1).Color = 'b';
h(2).Color = 'r';
h(3).Color = 'k';
...
xlim([1,n_trials+1]);
ylim([-0.06, 0]);
legend('target depth change', 'compaction history', 'measured depth change')
hold off
...
function [response, response_noisy] = comsol_response(comsol_model, action)
    global noise_sd;
    ...
    % add noise
    response_noisy = response + [noise_sd*randn; noise_sd*randn];
end
